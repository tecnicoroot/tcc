...
class Editar extends Component {
  ...
  async componentDidMount() {
    ...
    if (id) {
      ...
      this.setState({ convenio: data.data });
  ...
  salvar = async (convenio) => {
    const { id } = this.state;
    //console.info(convenio);  
    await api.put(`register/${id}`, convenio);
    ...
    return (
      <Row>
                ...
                <Col xl="4">
                  <Col xl="12" >
                    <FormGroup row className="botoes-tela-lista">
                      <h2><span>Editar de Convênio</span> </h2>
                    </FormGroup>
                  </Col>
                </Col>
        ...
        <Modal isOpen={this.state.modalRemover} toggle={this.fecharModalRemover}
          ...
          <ModalBody>
            Tem certeza que deseja excluir este convênio? {this.state.convenio.email}`?
            </ModalBody>
          <ModalFooter>
