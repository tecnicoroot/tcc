...
class CadastroConvenio extends Component {
  ...
  async componentDidMount(){
    ...
    if(id){
      ...
      const { data } = await axios.get(`http://localhost:3100/convenio/${id}` );

      this.setState({convenio: data});
      if(this.state.convenio.pessoa === "física"){
  ...
  salvar = async (convenio) => {
    ...
    if(this.state.profileImg !== ""){
      convenio.logo =  this.state.profileImg;
    };
    if(id){
      await axios.put(`http://localhost:3100/convenio/${id}`, convenio);
      localStorage.setItem("cadastroConvenio", "update");
    }else{
      await axios.post("http://localhost:3100/convenio", convenio);
      localStorage.setItem("cadastroConvenio", "add");
    }
  ...
  excluirconvenio = async (id) =>{
    if(id){
     await axios.delete(`http://localhost:3100/convenio/${id}`);
    }
    localStorage.setItem("cadastroConvenio", "del");
    this.props.history.push("/convenio");
  }
  imageHandler = (e) => {
    ...
    return (
      <Row>
          ...
          <CardHeader>
                <FormGroup row>
                  ...
                  <Col xl="12" >
                    ...
                    <h2><span>Cadastro Convênio</span> </h2>
                    </FormGroup>
                  </Col>
                  </Col>
        ...
        <Modal isOpen={this.state.modalRemover} toggle={this.fecharModalRemover}
            ...
            <ModalBody>
            Tem certeza que deseja excluir este convênio? {this.state.convenio.razaoSocial }`?
            </ModalBody>
            <ModalFooter>
