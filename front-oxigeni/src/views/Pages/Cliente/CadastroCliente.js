...
class CadastroCliente extends Component {
  ...
  async componentDidMount(){
    ...
    if(id){
      ...
      const { data } = await axios.get(`http://localhost:3100/cliente/${id}` );

      this.setState({cliente: data});
      if(this.state.cliente.pessoa === "física"){
  ...
  salvar = async (cliente) => {
    ...
    if(this.state.profileImg !== ""){
      cliente.logo =  this.state.profileImg;
    };
    if(id){
      await axios.put(`http://localhost:3100/cliente/${id}`, cliente);
      localStorage.setItem("cadastroCliente", "update");
    }else{
      await axios.post("http://localhost:3100/cliente", cliente);
      localStorage.setItem("cadastroCliente", "add");
    }
  ...
  abrirModalRemover = () => {
   ...
   excluircliente = async (id) =>{
     if(id){
      await axios.delete(`http://localhost:3100/cliente/${id}`);
     }
     localStorage.setItem("cadastroCliente", "del");
     this.props.history.push("/cliente");
   }
   imageHandler = (e) => {
    ...
    return (
      <Row>
          ...
          <CardHeader>
                <FormGroup row>
                  ...
                  <Col xl="12" >
                    ...
                    <h2><span>Cadastro Cliente / Fornecedor</span> </h2>
                    </FormGroup>
                  </Col>
                  </Col>
        ...
        <Modal isOpen={this.state.modalRemover} toggle={this.fecharModalRemover}
            ...
            <ModalBody>
            Tem certeza que deseja excluir este cliente? {this.state.cliente.razaoSocial }`?
            </ModalBody>
            <ModalFooter>
              <Button color="danger" onClick={ () => this.excluircliente(this.state.cliente.id)}>Sim</Button>{' '}
